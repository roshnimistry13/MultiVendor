...
jQuery(document).ready(function()
{
	...
	url = base_url + "Admin/Brand/bindDataTable";
	target = [0,3];
...
$("form[id='brand-form']").validate(
	{
		...
		messages: {
			text_brand_name		: {required: "Please enter brand name"},
		},
...
//Update Brand Status
function updateBrand(id,is_active)
{
	if(is_active == 1)
		var isActive = "Enable";
	else if(is_active == 0)
	...
	Swal.fire(
		...
		}).then((result) =>
		{
			if (result.value)
			{
				...
				$.ajax(
					{
						"url" : base_url + "Admin/Brand/updateStatus",
						type: 'post',
						dataType: 'json',
						data: updateStatus,
