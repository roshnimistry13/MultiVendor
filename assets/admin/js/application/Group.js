...
{
	table_name = 'groupDatatable';
	url = base_url + "Admin/Group/bindDataTable";
	target = [0,3];
...
$("form[id='productgroup-form']").validate(
	{
		...
		messages: {
			text_product_group		: {required: "Please enter product group"},
		},
...
//Update Group Status
function updateGroup(id,is_active)
{
	if(is_active == 1)
		var isActive = "Enable";
	else if(is_active == 0)
	...
	Swal.fire(
		...
		}).then((result) =>
		{
			if (result.value)
			{
				...
				$.ajax(
					{
						"url" : base_url + "Admin/Group/updateStatus",
						type: 'post',
						dataType: 'json',
						data: updateStatus,
						success: function (data)
						{
							if(data.status=="success")
							{
								...
								var status1 = "Delete";
								Swal.fire("Success", "Successsfully "+status1+"d Group.", "success");
