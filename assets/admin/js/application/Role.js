...
{
	table_name = 'roleDatatable';
	url = base_url + "Admin/Role/bindDataTable";
	target = [0,3];
...
$("form[id='role-form']").validate(
	{
		...
		messages: {
			text_role_name		: {required: "Please enter role name"},
		},
...
function updateRole(id,is_active)
{
	if(is_active == 1)
		var isActive = "Enable";
	else if(is_active == 0)
	...
	Swal.fire(
		...
		}).then((result) =>
		{
			if (result.value)
			{
				...
				$.ajax(
					{
						"url" : base_url + "Admin/Role/updateStatus",
						type: 'post',
						dataType: 'json',
						data: updateStatus,
